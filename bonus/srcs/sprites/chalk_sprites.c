	...
	for (row = 0; row < game->map_height; row++)
	{
		for (col = 0; game->map[row] && game->map[row][col]; col++)
		{
			if (game->map[row][col] == 'c')
			{
				...
				// Give each chalk slightly different animation timing
				game->chalk_sprites[game->chalk_sprite_count].time = game->chalk_sprite_count * 0.5;
				game->chalk_sprites[game->chalk_sprite_count].amplitude = CHALK_AMPLITUDE;
				game->chalk_sprites[game->chalk_sprite_count].frequency = CHALK_FREQUENCY + (game->chalk_sprite_count * 0.1);
...
{
	// Update animation time only
	sprite->time += delta_time;
	
	// Don't modify world coordinates (x, y) - keep them fixed!
	// Animation offset will be applied during rendering
...
void	render_chalk_sprite(t_game *game, t_chalk_sprite *sprite)
{
	...
	// Calculate sprite size based on distance (same formula as walls)
	if (distance < 1.0)
		distance = 1.0;
	sprite_size = (int)(SPRITE_SCALE_FACTOR * CUBE / distance);
	if (sprite_size < 2)
		sprite_size = 2;
	if (sprite_size > HEIGHT / 2)
	...
	start_x = screen_x - sprite_size / 2;
	end_x = screen_x + sprite_size / 2;
	start_y = HEIGHT / 2 - sprite_size / 2 + pitch_offset;  // Apply same pitch offset as walls
	end_y = HEIGHT / 2 + sprite_size / 2 + pitch_offset;   // Both Y positions get pitch offset
	...
	for (y = start_y; y < end_y; y++)
	{
		for (x = start_x; x < end_x; x++)
		{
			...
			if (sprite_size > 0)
			{
				tex_x = (x - start_x) * sprite->width / sprite_size;
				tex_y = (y - start_y) * sprite->height / sprite_size;
			}
			else
			{
				tex_x = 0;
			...
			if (tex_x >= 0 && tex_x < sprite->width && tex_y >= 0 && tex_y < sprite->height)
			{
				...
				if (tex_index >= 0 && tex_index < sprite->width * sprite->height)
				{
					...
					// For XPM files, MinilibX handles transparency automatically
					// We need to check for the actual transparent color value
					// In most cases, transparent pixels will be 0x00000000 or a specific color
					// Let's render all non-zero pixels (skip only pure black/transparent)
					if (pixel_color != 0)
					{
